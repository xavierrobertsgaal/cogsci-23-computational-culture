# Runs the agent-based simulation replicating Giuliano and Nunn's model of cultural transmission
# That is, only includes MF and IL agents, and no MB agents
...
class Agent:
    ...
    def observe(self, other):
        self.observed = other.action
    
    def act(self, env) -> None:
        if self.strategy == 'MF':
            self.action = self.observed
            ...
            raise ValueError('Invalid strategy. Can only be "MF" or "IL"')
...
class World:
    def __init__(self, rng, Kappa: float = 0.5, Delta: float = 0.2) -> None:
        self.agents = []
        self.gen = 0
        self.rng = rng
    ...
    def add_agent(self, id: int, **kwargs) -> None:
        kwargs.setdefault('strategy', self.rng.choice(['MF', 'IL']))
        self.agents.append(
    ...
    def step(self) -> None:
        ...
        if self.rng.choice([True, False], p=[0.95, 0.05]):
            ...
            self.add_agent(id = self.get_agent_count() + self.gen + 1,
                            strategy = self.rng.choice(['MF', 'IL']))
...
def simulate(path: str, size: int, gens: int, Delta: float, Kappa: float, seed: int = 42) -> pd.DataFrame:
    """
    ...
    """
    # print(f"Running simulation with parameters size={size}, gens={gens}, Delta={Delta}, Kappa={Kappa}, seed={seed}") 
    rng = np.random.default_rng(seed=seed) # LOOK INTO WHETHER I CAN PREGENERATE
    env = World(rng=rng, Delta=Delta, Kappa=Kappa)
    for i in range(size):
        env.add_agent(id = i)
    df = env.run(gens)
    df.to_csv(f"{path}_size={size}_gens={gens}_Delta={Delta}_Kappa={Kappa}_seed={seed}.csv", index=False)
    return df
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='sims_gn/sim', help='Path prototype for simulation results')
    ...
    if args.params_path is not None:
        ...
        with joblib_progress("Simulation progress", total=len(params)):
            Parallel(n_jobs=args.ncores)(
                delayed(simulate)(path=args.path, size=args.size, gens=args.gens, Delta=D, Kappa=K, seed=args.seed) 
                for D, K in zip(params['Delta'], params['Kappa'])
            )
    else:
        with joblib_progress("Simulation progress", total=args.ns**2):
            Parallel(n_jobs=args.ncores)(
                delayed(simulate)(path=args.path, size=args.size, gens=args.gens, Delta=D, Kappa=K, seed=args.seed) 
